...
var algo = document.querySelector("#createdBy");
algo.innerHTML = 'Aplicación creada por Cruz';
...
function buildToDo(todo, index) {
  // Tu código acá:
  const body = document.querySelector('body');     // guardamos en esa variable el body
  var toDoShell = document.createElement('div');  // creamos e esa variable un div
  body.appendChild(toDoShell);                   // creamos un div en el body

  toDoShell.className = 'toDoShell';              // creamos la clase toDoShell
  var toDoText = document.createElement('span');  // creamos un elemento span alojado en la variable
  body.appendChild(toDoText);                     // lo agregamos al body
  toDoText.innerHTML = todo.description;          // seteamos el innerHTML accediento al parametro y a la propiedad de ese parametro
  toDoText.id = index;                            // le asignamos a ToDoText el id index pasado por parametro
  
  if ( todo.complete === true) {       // si la condicion se cumple
    toDoText.class = 'completeText';   // a toDoText le asigna la clase completetext
  } 
  return toDoShell
}
...
function buildToDos(toDos) {
  // Tu código acá:
return nuevoArray = toDos.map(buildToDo);
}
...
function displayToDos() {
  // Tu código acá:
var toDoContainer = document.querySelector('#toDoContainer');  // guardo en toDoContainer el elemento que tenga el id #toDoContainer
toDoContainer.innerHtml = '';     // a eso que me traje que pongo el texto vacio
var iterar = buildToDos(toDoItems);  //guardo en la variable iterar la llamada de esa funcion
for (let i = 0; i < iterar.length; i++) {
     iterar.map(appendChild(toDoContainer))}
    }
...
function addToDo() {
  // Tu código acá:
var nuevoToDo = new ToDo(toDoInput); // creo un nuevoToDO usando la claseToDo
toDoItems.push(nuevoToDo);           // agrego el obj recien creado al array toDoItems
toDoInput = '';
displayToDos();
}
